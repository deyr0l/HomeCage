...
| `CAMERA` | HomeCage 内のライトボタン用。torch mode のみを使い、カメラ画像は取得しません。 |
| `ACCESS_COARSE_LOCATION` / `ACCESS_FINE_LOCATION` | サーバーが要求した場合の任意の位置情報レポート。 |
| `PACKAGE_USAGE_STATS` | consumer モード用の任意レイヤー。Accessibility の前面アプリ情報が不完全な場合の補助信号です。 |
| `SYSTEM_ALERT_WINDOW` | consumer モード用の任意レイヤー。ブロック画面に通常の Android overlay を使えます。 |
...
使用しません: 連絡先、SMS、カメラ画像取得、マイク、notification listener、VPN、`QUERY_ALL_PACKAGES`。
...
```sh
# server ディレクトリで root として実行してください。
cd server
./install-service.sh
```

Alpine/OpenRC では先に依存パッケージを入れてください: `apk add python3 py3-pip py3-virtualenv openrc`。スクリプトは `homecage` service user/group を作成し、空きポートを探して、自動起動を有効にします。
